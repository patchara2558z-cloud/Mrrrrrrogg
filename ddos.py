...
menu_border = '''
 ...
 └─┬─────────┤ Network Scanner ├─────────┬──────────────┤ Osint ├──────────────┬────────────┤ Utilities ├────────────┴─
   │         └─────────────────┘         │              └───────┘              │            └───────────┘
   ├─ [01] Website Vulnerability Scanner ├─ [10] D0x Create                    ├─ [20] Phishing Attack
   ├─ [02] Website Info Scanner          ├─ [11] D0x Tracker                   ├─ [21] Password Zip Cracked Attack
   ├─ [03] Website Url Scanner           ├─ [12] Get Image Exif                ├─ [22] Password Hash Decrypted Attack
   ├─ [04] Ip Scanner                    ├─ [13] Google Dorking                ├─ [23] Password Hash Encrypted
   ├─ [05] Ip Port Scanner               ├─ [14] Username Tracker              ├─ [24] Search In DataBase
   └─ [06] Ip Pinger                     ├─ [15] Email Tracker                 ├─ [25] Dark Web Links
                                         ├─ [16] Email Lookup                  └─ [26] Ip Generator
                                         ├─ [17] Phone Number Lookup
                                         ├─ [18] Ip Lookup
                                         └─ [19] Instagram Account
'''
...
def main():
    ...
    while True:
        print_menu()
        print(f"{Fore.RED}┌───({Fore.WHITE}{username}@redtiger{Fore.RED}){Fore.RED}-[{Fore.WHITE}~/Windows/Menu-1{Fore.RED}] ")
        user_input = input(f"{Fore.RED}└─{Fore.WHITE}$ ")
